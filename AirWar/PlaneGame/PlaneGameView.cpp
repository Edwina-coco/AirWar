...
void CPlaneGameView::AI()
{
	...
	if (m_Circle != NULL)
	{
		...
		for (bPos11 = m_ObjList[enBall].GetHeadPosition(); (bPos22 = bPos11) != NULL;)  //遍历enball，判断哪个enball与飞机发生碰撞，然后删除即可
		{
			CBall* pBall = (CBall*)m_ObjList[enBall].GetNext(bPos11);
			CRect bRect = pBall->GetRect();
			CRect tmpRect;
			if (tmpRect.IntersectRect(&bRect, mRect)) //此处实现碰撞检测机制  IntersectRect函数用来判断两个矩形是否有交接
			{
				//添加爆炸效果
